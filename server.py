...
def previous_reading():
    ...
    data = cursor.fetchall()
    if c is None:
       return ("No record found. Selected other equipment. ")    
...
def select_e():
    ...
    venderid = request.args.get('venderid')
    equipmentid = request.args.get('equipmentid')
    print ("print-equ ven",equipmentid, venderid)
    cursor = mysql.connection.cursor()
    ...
    #cursor.execute('SELECT equ_id,equ_name,equ_make,equ_model,equ_serialno,equ_asset, STR_TO_DATE(start_date, "/%Y-/%m-/%d") as start_date, active FROM equipment where vender_id=%s',(venderid))
    result = cursor.fetchall()
